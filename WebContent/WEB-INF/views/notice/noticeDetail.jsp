<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8" import="notice.model.vo.Notice, notice.model.vo.Comments, java.util.ArrayList" %>
<% 
...
<body>
	...
	<div class="outer">
		...
		<div class="tableArea">
			<form action="noticeUpdateForm.no" id="detailForm" name="detailForm" method="post">
				...
				<label id="title"><%= notice.getNoticeTitle() %></label><br><br>
					<%-- <%= notice.getMemNick() %> --%>
				<label id="writer"><%= notice.getMemNick() %></label>
				<input type="hidden" size="50" name="nickName" value="<%= notice.getMemNick() %>"> 
				<br>				
				<input type="hidden" name="date" value="<%= notice.getNoticeDate() %>">
				<label class="jj"><%= notice.getNoticeDate() %></label> &nbsp;&nbsp; <label class="jj">조회&nbsp;<%= notice.getNoticeCount() %></label>
					<br>
					<hr><br>
					<textarea name="content" id="tcontent" cols="60" rows="20" style="resize:none;" readonly><%= notice.getNoticeContent() %></textarea>
				
				<br><br><br>
				
				<div align="center">
					<% if(loginMember != null && loginMember.getMemId().equals(notice.getNoticeWriter())){ %> 
	...
	<%-- 			<input type="button" onclick="location.href='javascript:history.go(-1);'" id="cancelBtn" value="뒤로"> --%>
					<input type="button" onclick="location.href='list.no'" id="cancelBtn" value="목록">
				</div>
			</form>
	...
	<br><br><br>
		...
		$(function(){
			$('#addComments').on('click', function(){
				...
				$.ajax({
					url: 'insertComments.no',
					data:{writer:writer, no:no, content:content},
