...
<% 
	Fboard sb = (Fboard)request.getAttribute("sboard");
...
<style>
	...
	#region{color: rgb(5, 84, 242); font-size: 15px; font-weight: bold;}
...
<body>
	...
	<div class="outer">
		<br>
		<h4>커뮤니티 > <%= sb.getCateName() %></h4>
		<div class="tableArea">
			<form action="boardUpdateForm.st" id="detailForm" name="detailForm" method="post">
				<input type="hidden" size="50" name="no" value="<%= sb.getBoardNo() %>">
 				<input type="hidden" size="50" name="id" value="<%= sb.getId() %>">  
				<input type="hidden" size="50" name="title" value="<%= sb.getBoardTitle() %>">
				<label id="title"><%= sb.getBoardTitle() %></label><br><br>
				<label id="nickName"><%= sb.getMemNick() %></label>
	 			<input type="hidden" size="50" name="nickName" value="<%= sb.getMemNick() %>"><br>
	 			<label id="region"><%= sb.getRegion() %></label><br>
	 			<input type="hidden" size="50" name="region" value="<%= sb.getRegion() %>">			
<%-- 				<input type="hidden" name="date" value="<%= fb.getModifyDate() %>">				 --%>
				<label class="jj"><%= sb.getModifyDate() %></label> &nbsp;&nbsp; <label class="jj">조회&nbsp;<%= sb.getBoardView() %></label>
					<br>
					<hr><br>
					<textarea name="content" id="tcontent" cols="60" rows="20" style="resize:none;" readonly><%= sb.getBoardContent() %></textarea>
				
				<br><br><br>
				
				
				<div align="center">
						<% if(loginMember != null && sb.getMemNick().equals(loginMember.getMemNick())){ %> 
...
<%-- 				<input type="button" onclick="location.href='javascript:history.go(-1);'" id="cancelBtn" value="뒤로"> --%>
					<input type="button" onclick="location.href='list.st'" id="cancelBtn" value="목록">
				</div>
			</form>
	...
	<br><br><br>
		...
		$(function(){
			$('#addComments').on('click', function(){
				...
				var stNo = <%= sb.getBoardNo() %>;
				var content = $('#commentsContent').val();
				
				$.ajax({
					url: 'insertComments.st',
					data:{writer:writer, stNo:stNo, content:content},
			...
			$('#deleteNoBtn').on('click', function(){
				...
				if(bool){
					location.href='<%= request.getContextPath() %>/delete.st?stNo=' + <%= sb.getBoardNo() %>;
				}
			})
		</script>
	</div>
